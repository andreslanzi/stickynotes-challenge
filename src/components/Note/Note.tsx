...
function Note({ note }: NoteProps) {
  ...
  const { updateNoteText, deleteNote, bringToFront, boardRef } = useNotes()
  ...
  const handleDelete = () => {
    // If focus was inside this note, send it to the board landmark instead
    // of letting it fall back to <body> once the note unmounts.
    ...
    >
      ...
      >
        ...
        <button
          ...
          className="flex h-5 w-5 items-center justify-center rounded text-xs leading-none text-slate-600/70 hover:bg-black/10 hover:text-red-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-700"
        >
          X
        </button>
      </header>
      ...
      <textarea
        ...
        className="note-scrollbar min-h-0 w-full flex-1 resize-none bg-transparent px-3 pt-2 pb-3 text-sm leading-snug outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-slate-700 placeholder:text-slate-500/60 placeholder:italic"
      />
      <div
        {...resizeProps}
